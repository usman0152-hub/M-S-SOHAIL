...
class AccountMove(models.Model):
    ...
    invoice_line_ids = fields.One2many('account.move.line', 'move_id', string='Invoice lines',
        ...
        states={'draft': [('readonly', False)]})
    # new line id for show separate bill fields in move line by usman
...
class AccountMoveLine(models.Model):
    ...
    # MS total sale purchase account fields
    ...
    carton_value = fields.Float("Carton Value")
    total_without = fields.Float("Total Without")
    ...
    # dollar compute formula by usman
    ...
    def _compute_analytic_tag_ids(self):
        ...
        # remove  price unit quantity , and discount coded by usman
    def _get_price_total_and_subtotal(self, labour_rate=None, carton_value=None, total_without=None, currency=None,
                                          product=None, partner=None, taxes=None, move_type=None):
            self.ensure_one()
            return self._get_price_total_and_subtotal_model(
                labour_rate=self.labour_rate if labour_rate is None else labour_rate,
                carton_value=self.carton_value if carton_value is None else carton_value,
                total_without=self.total_without if total_without is None else total_without,
        ...
        # remove price_unit , quality and discount coded by usman
    @api.model
    def _get_price_total_and_subtotal_model(self, labour_rate, carton_value, total_without, currency, product,
                                                partner, taxes, move_type):
            ''' This method is used to compute 'price_total' & 'price_subtotal'.

            :param labour_rate:  The current Labour Rate.
            :param carton_value:    The Carton Value.
            :param total_without:    The current Total Without.
            ...
            # original code edited by usman
            # line_discount_price_unit = price_unit * (1 - (discount / 100.0))
            # subtotal = quantity * line_discount_price_unit
            line_discount_price_unit = total_without + labour_rate + carton_value
            subtotal = line_discount_price_unit

            # Compute 'price_total'.
            # remove quantity from compute all edited by usman
            if taxes:
    ...
    def _onchange_amount_currency(self):
        ...
        # @api.onchange('quantity', 'discount', 'price_unit', 'tax_ids') edited by usman
    @api.onchange('labour_rate', 'carton_value', 'total_without', 'tax_ids')
    ...
    def create(self, vals_list):
        # OVERRIDE
        # BUSINESS_FIELDS = ('price_unit', 'quantity', 'discount', 'tax_ids') edited by usman
        ACCOUNTING_FIELDS = ('debit', 'credit', 'amount_currency')
        BUSINESS_FIELDS = ('labour_rate', 'carton_value', 'total_without', 'tax_ids')
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if any(vals.get(field) for field in ACCOUNTING_FIELDS):
                    price_subtotal = self._get_price_total_and_subtotal_model(
                        ...
                        # code edited by usman
                        vals.get('labour_rate', 0.0),
                        vals.get('carton_value', 0.0),
                        vals.get('total_without', 0.0),
                        currency,
        ...
        for line in self:
            ...
            if any(field in cleaned_vals for field in ACCOUNTING_FIELDS):
                ...
                to_write.update(line._get_price_total_and_subtotal(
                    ...
                    labour_rate=to_write.get('labour_rate', line.labour_rate),
                    carton_value=to_write.get('carton_value', line.carton_value),
                    total_without=to_write.get('total_without', line.total_without),
                ))
