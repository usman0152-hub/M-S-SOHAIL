...
class SaleOrder(models.Model):
    ...
    def onchange_partner_id(self):
        ...
        values = {
            ...
            # Hide because shipping id is copy from delivery address and client does not want this edited by usman
        }
...
# compute code for price subtotal edit by usman
    @api.depends('total_without')
    def _compute_amounted(self):
        for record in self:
    ...
    # usman MS fields
    price_unit = fields.Float('Unit Price', compute="_compute_price", required=True, digits='Product Price', default=0.0)
    ...
    total_without = fields.Float(string="Total Without", compute="_compute_total_without")
    form = fields.Char(string="Form E no", store=True)
    container = fields.Char(string="Container", store=True)

    price_subtotal = fields.Monetary(compute='_compute_amounted', string='Subtotal', readonly=True, store=True)
    ...
    # usman price_unit compute formula
    ...
    def _compute_total_without(self):
        for record in self:
            record.total_without = record.product_uom_qty * record.price_unit
    ...
    def _get_invoice_line_sequence(self, new=0, old=0):
     ...
     # MS sale fields move to invoice line edited by usman
